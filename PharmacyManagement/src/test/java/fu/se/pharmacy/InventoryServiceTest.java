...
public class InventoryServiceTest {
    ...
    // FIX: bổ sung test cho method này — trước đây tồn tại nhưng không được gọi ở đâu
    // (Refund tự sửa kho riêng), nay RefundServiceImpl đã gọi thật nên cần test bảo vệ.

    @Test
    @DisplayName("restoreForCancelledSale - hoan lai dung so luong da tru, ghi transaction SALE_RETURN")
    public void testRestoreForCancelledSale_RestoresQuantityAndLogsReturn() {
        Sale sale = new Sale();
        sale.setSaleId(1);
        sale.setBranchId(1);
        sale.setPharmacistId(5);
        ...
        InventoryBatch batch = new InventoryBatch();
        batch.setInventoryBatchId(1);
        batch.setMedicineId(1);
        batch.setQuantityOnHand(0);
        batch.setStatus("DISPOSED"); // da het hang do sale truoc do tru ve 0

        InventoryTransaction saleTx = new InventoryTransaction();
        saleTx.setBranchId(1);
        saleTx.setMedicineId(1);
        ...
        when(inventoryBatchRepository.findById(1)).thenReturn(Optional.of(batch));
        when(inventoryBatchRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
        when(transactionRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        inventoryService.restoreForCancelledSale(1);

        verify(inventoryBatchRepository).save(argThat(b ->
                b.getQuantityOnHand() == 10 && "AVAILABLE".equals(b.getStatus())));
        verify(transactionRepository).save(argThat(tx ->
                tx.getQuantityChange() == 10 && "SALE_RETURN".equals(tx.getTransactionType())));
    }

    @Test
    @DisplayName("restoreForCancelledSale - khong co giao dich SALE nao - khong lam gi")
    public void testRestoreForCancelledSale_NoSaleTransactions_DoesNothing() {
        Sale sale = new Sale();
        sale.setSaleId(2);
        sale.setBranchId(1);

        when(saleRepository.findById(2)).thenReturn(Optional.of(sale));
        when(transactionRepository.findByReferenceIdAndTransactionType(2, "SALE"))
                .thenReturn(List.of());

        inventoryService.restoreForCancelledSale(2);
